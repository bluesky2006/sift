...
function readCookie(req, name) {
  ...
  for (const part of raw.split(';')) {
    ...
    if (i > 0 && part.slice(0, i).trim() === name) {
      // another service on this IP can set a cookie of this name; a malformed one is no session
...
// to grind the password down. Counted per address, so one device guessing can't lock the
// others out, with a cap across all of them so several devices together can't grind either.
const attempts = new Map();     // address -> times
const everyone = [];
const LOCKOUT_MAX = 10;
const LOCKOUT_ALL = 50;
const LOCKOUT_WINDOW_MS = 5 * 60 * 1000;

function expire(times) {
  const cutoff = Date.now() - LOCKOUT_WINDOW_MS;
  while (times.length && times[0] < cutoff) times.shift();
  return times;
}
function lockedOut(addr) {
  for (const [a, times] of attempts) if (!expire(times).length) attempts.delete(a);
  return (attempts.get(addr) || []).length >= LOCKOUT_MAX || expire(everyone).length >= LOCKOUT_ALL;
}
function recordFailure(addr) {
  if (!attempts.has(addr)) attempts.set(addr, []);
  attempts.get(addr).push(Date.now());
  everyone.push(Date.now());
}
// Counted before the slow check, not after it, so guesses sent all at once can't slip past
// the limit while the first ones are still being checked. A right password clears that
// address's count.
function startAttempt(addr) {
  if (lockedOut(addr)) return false;
  recordFailure(addr);
  return true;
}
function clearFailures(addr) { attempts.delete(addr); }
